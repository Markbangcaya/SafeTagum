                        ...
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="form-group col-3">
                                    ...
                                    <has-error :form="form" field="type_of_disease.id" />
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="row">
                                <div class="col-4">
                                    <div v-if="form.barangay" class="form-group">
                                        ...
                                        <has-error :form="form" field="streetpurok.name" />
                                    </div>
...
export default {
            ...
            if (this.form.barangay) {
                axios.get(`/api/barangay/${this.form.barangay.id}`)
                    .then(response => {
                        ...
                        // Automatically select the street/purok after loading puroks
                        this.form.streetpurok = this.option_puroks.find((option) =>
                            option.name === this.row['street/purok']
                        );
                    })
                    .catch(error => {
    ...
    watch: {
        // option_puroks(newVal) {
        //     if (newVal.length > 0) {
        //         this.form.streetpurok = newVal.find((option) => option.name === this.row['street/purok']);
        //     }
        // },
        row: function () {
            ...
            // Load puroks for the selected barangay
            if (this.form.barangay) {
                this.loadPuroks();
            }
            // Set the marker position based on the latitude and longitude from the row data
    ...
    mounted() {
        this.loadDisease();
        this.loadBarangay();
        $('#edit-patient').on('shown.bs.modal', () => {
            this.loadGeoJSON();
        });
    }
}
