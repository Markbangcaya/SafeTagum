        ...
        <div class="content">
            <div class="container-fluid">
                <div class="row justify-content-center">
                        ...
                        <div class="card">
                            ...
                            <div class="card-body table-responsive p-0">
                                <table class="table table-head-fixed text-nowrap">
                                    ...
                                    <tbody>
                                        <tr v-for="(data, index) in option_disease.data" :key="index">
                                            <td>{{ data.id }}</td>
                                            <td>{{ data.name }}</td>
                                            <td class="text-right">
                                ...
                                <ul class="pagination pagination-sm m-1 float-right">
                                    <li class="page-item" v-for="(link, index) in option_disease.links" :key="index">
                    ...
                    <edit-modal @getData="getData" :row="selected_disease" :page="current_page"></edit-modal>
                </div>
            </div>
        </div>
    </div>
...
<script>
import addModal from "./AddDisease.vue";
import EditModal from "./EditDisease.vue";
export default {
    components: {
        addModal,
        EditModal,
                ...
                axios.get(page, {
                    ...
                    .then(response => {
                        if (response.data.data) {
                            this.option_disease = response.data.data;
                        }
                    }).catch(error => {
                        this.error = error;
                        toast.fire({
        ...
        remove(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.delete('/api/disease/delete/' + id)
                        .then(response => {
                            Swal.fire(
                                'Deleted!',
