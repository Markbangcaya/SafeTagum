                        ...
                        <div class="card">
                            ...
                            <div class="card-body table-responsive p-0">
                                <table class="table table-head-fixed text-nowrap">
                                    ...
                                    <tbody>
                                        <tr v-for="(data, index) in option_barangays.data" :key="index">
                                            <td>{{ data.id }}</td>
                                            <td>{{ data.name }}</td>
                                            <!-- <td>{{ data.email }}</td> -->
                                            <td class="text-right">
                                ...
                                <ul class="pagination pagination-sm m-1 float-right">
                                    <li class="page-item" v-for="(link, index) in option_barangays.links" :key="index">
...
<script>
import addModal from "./AddBarangay.vue";
import EditModal from "./EditBarangay.vue";
export default {
    components: {
        addModal,
        EditModal,
                ...
                axios.get(page, {
                    ...
                    .then(response => {
                        if (response.data.data) {
                            this.option_barangays = response.data.data;
                        }
                    }).catch(error => {
                        this.error = error;
                        toast.fire({
        ...
        remove(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.delete('/api/barangay/delete/' + id)
                        .then(response => {
                            Swal.fire(
                                'Deleted!',
